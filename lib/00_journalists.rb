...
def twitter_size(twitter)
  size = twitter.length
  puts "Il y a #{size} handle dans cette array"
end

# #Quelle est le handle le plus court de cette liste ?
def twitter_shorten(twitter)
  minimum = twitter.min{|a, b| a.length <=> b.length}
  puts "Le handle le plus court est : #{minimum}"
end

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
caracteres = twitter.select{|num| num.length == 6 }.length
puts "#{caracteres} handle contiennent 5 caractères"

#Combien commencent par une majuscule (première lettre juste après le @) ?
uppercase = twitter.count{|u| u =~ /^@[[:upper:]]/}
puts "Il y a #{uppercase} handles qui commencent par un majuscule"
...
puts "Par taille des handles"
puts twitter.sort_by {|x| x.length}

# Quelle est la position dans l'array de la personne @epenser ?
position = twitter.index("@epenser")
puts "@epenser est en position : #{position}"
...
def perform(twitter)
  twitter_size(twitter)
  twitter_shorten(twitter)
end
